...
async def update_genero(genero_id: str, genero: Genero) -> dict:
    result = collection.update_one({"_id": ObjectId(genero_id)}, {"$set": genero.dict()})
    if result.modified_count == 1:
        return {"message": "Genero atualizada com sucesso"}

async def delete_genero(genero_id: str) -> dict:
    result = collection.delete_one({"_id": ObjectId(genero_id)})
    if result.deleted_count == 1:
        return {"message": "Genero deletada com sucesso"}
